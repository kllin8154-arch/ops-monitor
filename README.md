...
公开仓库只保留源码、运行所需的公开配置模板、公开截图和 README。以下内容不属于开源范围：

- 内部实施报告、交付审查记录、闭环验证记录和阶段建设文档。
- Agent 约束、个人工作流规则、路线图、产品草案和内部测试计划。
- 运行时数据、审计日志、健康报告、锁文件、真实 targets、备份配置和历史漂移副本。
- `.env`、私钥、证书、真实密码、token、cookie 或任何生产环境密钥。

如果这些文件曾经被 Git 跟踪，仅写入 `.gitignore` 不会自动移除，需要在首次公开推送前从 Git 索引中清理。
